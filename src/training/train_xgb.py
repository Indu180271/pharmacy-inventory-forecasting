...
drift_detected_total = Counter(
    ...
    "Number of data drift alerts detected"
)
...
experiment_name = "sku_xgb_forecasting"
...
def train_sku_model(
    ...
    with mlflow.start_run(run_name=f"sku_{sku_id}") as run:
        ...
        # ------------------------------------------------------------------
        # Save local model under MODELS_PATH and log artifact
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # Log model to MLflow with signature + input_example
        #   - avoids warnings about missing signature/input_example
        #   - uses constant artifact_path="model"
        # ------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    try:
        ...
        training_success_total.inc()
        print("Training run SUCCESS")
    except Exception as e:
        training_failure_total.inc()
        print("Training FAILED:", e)
        raise e

    # Keep container alive so Prometheus can scrape
    while True:
        time.sleep(60)
